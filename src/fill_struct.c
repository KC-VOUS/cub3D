...
/*   Updated: 2025/09/17 19:37:02 by sdakhlao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_texture(t_details *details, char **tab)
{
	...
	while (tab[i])
	{
		...
		while (tab[i][j] == ' ')
			j++;
		if (!ft_strncmp(tab[i] + j, "NO", 2))
			details->north = ft_strtrim(tab[i] + j + 2, "\n");
		else if (!ft_strncmp(tab[i] + j, "SO", 2))
			details->south = ft_strtrim(tab[i] + j + 2, "\n");
		else if (!ft_strncmp(tab[i] + j, "WE", 2))
			details->west = ft_strtrim(tab[i] + j + 2, "\n");
		else if (!ft_strncmp(tab[i] + j, "EA", 2))
			details->east = ft_strtrim(tab[i] + j + 2, "\n");
		i++;
	}
}
